{if $_action neq 'create' and $resource->{$field}}
    <div class="assets-preview">
        <div class="image-preview pull-left">
            <a href="{$resource->get_attachment($field)}?{$smarty.now}" data-toggle="lightbox" title="{$_labels.files.preview|escape}: {$resource->{$field}|escape}">
                <img src="{$resource->get_attachment($field)}?{$smarty.now}" class="img-thumbnail" style="max-width: 256px; max-height: 256px;" alt="{$resource->{$field}|escape}" />
            </a>
        </div>
        <div class="file-preview pull-left">
            <blockquote>
                <h4 class="text-thin">{$attr.default}</h4>
                <p><a href="{$resource->get_attachment($field)}" class="btn btn-link no-padding-left" target="_blank"><i class="glyphicon glyphicon-download-alt"></i> {$_labels.files.download} ( {{{{$resource->get_attachment($field)|replace:$_urls.root:$_urls.root_path}|filesize} / 1024}|string_format:"%.2f"} KB )</a></p>
            </blockquote>
        </div>
    </div>
{/if}
