...
{foreach from=$sections key=section_key item=section}
  {if $section.meta.show|default:true}
    {if $section.fields|default:false}
      ...
      <h3 class="no-margin-top text-thin"><i class="glyphicon glyphicon-{$section.meta.icon}"></i> {$section.meta.title}
      </h3>
              ...
              <td>
                ...
                {else}
                  <div class="well well-small">
                    Missing field type show template: <code>{$_mode}views/_shared/entities/types/{$attr.type}
                      /show.html.tpl</code>
                  </div>
                {/if}
              </td>
